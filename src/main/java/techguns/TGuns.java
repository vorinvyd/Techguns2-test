...
public class TGuns implements ITGInitializer {
	...
	public static void registerItems(RegistryEvent.Register<Item> event){
		...
		if(TGConfig.misc.debug) { //FIXME remove debug
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		float ndrDamage = TGConfig.srpIntegration.rebalance_nucleardeathray
				? 12.0f : 5.0f;
		nucleardeathray = new GenericGun("nucleardeathray", NDR_PROJECTILES, false, 5, 20, 50, ndrDamage, TGSounds.BEAMGUN_FIRE, TGSounds.LASERGUN_RELOAD, MAX_RANGE_SNIPER, 0.0f).setFiresoundStart(TGSounds.BEAMGUN_START).setMaxLoopDelay(10).setRecoiltime(10).setCheckRecoil().setBulletSpeed(100.0f).setAIStats(RANGE_MEDIUM, 40, 5, 5).setTexture("textures/guns/ndr").setPenetration(PENETRATION_MED).setDamageDrop(11, 11, ndrDamage).setHandType(GunHandType.TWO_HANDED).setTurretPosOffset(0, 0.04f, -0.19f).setCrossHair(EnumCrosshairStyle.TRI);
		...
		float[] tfgDamage = TGConfig.srpIntegration.rebalance_tfg
				? new float[]{ 70.0f, 130.0f } : new float[]{ 30.0f, 70.0f };
		tfg = new GenericGunCharge("tfg", TFG_PROJECTILES, false, 12, 20,45, tfgDamage[1], TGSounds.TFG_FIRE, TGSounds.BIOGUN_RELOAD, 100, 0.015f, 60.0f ,5).setChargeSound(TGSounds.TFG_CHARGE).setChargeFX("TFGChargeStart",-0.14f, -0.10f, 0.42f).setBulletSpeed(6.0f).setRangeTooltipType(RangeTooltipType.RADIUS).setPenetration(PENETRATION_MED_HIGH).setTexture("textures/guns/tfg").setAIStats(RANGE_SHORT, 30, 0, 0).setDamageDrop(8, 15, tfgDamage[0]).setMuzzleLight(0.2f, 1.0f, 0.2f).setMuzzleFlashTime(10).setRecoiltime(10).setForwardOffset(0.40f).setCrossHair(EnumCrosshairStyle.TRI_INV);
